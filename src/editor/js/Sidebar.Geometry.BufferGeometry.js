	...
	function update( object ) {
		...
		if ( geometry ) {
			...
			if ( index !== null ) {
				...
				containerAttributes.add( new UIText( ( index.count ).toString() ).setFontSize( '12px' ) );
			...
			for ( const name in attributes ) {
				...
				containerAttributes.add( new UIText( ( attribute.count ).toString() + ' (' + attribute.itemSize + ')' ).setFontSize( '12px' ) );
				containerAttributes.add( new UIBreak() );
			...
			if ( hasMorphTargets === true ) {
				...
				for ( const name in morphAttributes ) {
					...
					containerMorphAttributes.add( new UIText( ( morphTargets.length ).toString() ).setFontSize( '12px' ) );
